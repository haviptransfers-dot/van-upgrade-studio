...
import tourAthens from "@/assets/tour-acropolis.webp";
...
const AthensTour = () => {
  ...
  const title = t("seo.athens.title");
  const description = t("seo.athens.description");
  const url = `${SITE_URL}/athens-city-tour`;

  const includesKeys = ["stadium", "lycabettus", "zeus", "hadrian", "syntagma", "monastiraki"] as const;
  ...
  return (
    <>
      <Helmet>
        ...
        <meta property="og:image" content={`${SITE_URL}${tourAthens}`} />
      </Helmet>
      ...
      <div className="min-h-screen bg-background text-foreground">
              ...
              {includesKeys.map((k, i) => (
                ...
                >
                  ...
                  <h3 className="font-display text-lg md:text-xl font-semibold text-primary mb-1.5">
                    {t(`athensTour.includes.${k}.title`)}
                  </h3>
                  <p className="text-muted-foreground text-sm md:text-base">
                    {t(`athensTour.includes.${k}.desc`)}
                  </p>
                </motion.div>
              ))}
            </div>
        ...
        <section className="py-12 md:py-20 px-4">
          <div className="container mx-auto max-w-3xl">
            <h2 className="font-display text-3xl md:text-4xl font-bold text-center mb-10">
              {t("athensTour.whatIncludedTitle")}
            </h2>
        ...
        <section className="py-12 md:py-16 px-4 bg-card">
          <div className="container mx-auto max-w-4xl">
            <h2 className="font-display text-3xl md:text-4xl font-bold text-center mb-10">
              {t("athensTour.infoTitle")}
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-5">
              {[
                { icon: Clock, label: t("athensTour.duration"), val: t("athensTour.durationVal") },
                { icon: Users, label: t("athensTour.type"), val: t("athensTour.typeVal") },
                { icon: MapPin, label: t("athensTour.departure"), val: t("athensTour.departureVal") },
                { icon: Wallet, label: t("athensTour.price"), val: t("athensTour.priceVal") },
        ...
        <section className="py-16 md:py-20 px-4 bg-card border-t border-primary/20">
          <div className="container mx-auto max-w-2xl text-center">
            ...
            <h2 className="font-display text-2xl md:text-4xl font-bold mb-6">
              {t("athensTour.title")} · {t("athensTour.priceVal")}
            </h2>
            <a href={bookUrl} target="_blank" rel="noopener noreferrer">
              <Button
                size="lg"
