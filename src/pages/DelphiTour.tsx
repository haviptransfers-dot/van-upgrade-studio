...
const DelphiTour = () => {
  ...
  const title = t("seo.delphi.title");
  const description = t("seo.delphi.description");
  const url = `${SITE_URL}/delphi-private-tour`;

  const includesKeys = ["site", "museum", "amfissa", "arachova", "distomo"] as const;
      ...
      <div className="min-h-screen bg-background text-foreground">
              ...
              {includesKeys.map((k, i) => (
                ...
                >
                  ...
                  <h3 className="font-display text-lg md:text-xl font-semibold text-primary mb-1.5">
                    {t(`delphiTour.includes.${k}.title`)}
                  </h3>
                  <p className="text-muted-foreground text-sm md:text-base">
                    {t(`delphiTour.includes.${k}.desc`)}
                  </p>
                </motion.div>
              ))}
            </div>
        ...
        <section className="py-12 md:py-20 px-4">
          <div className="container mx-auto max-w-3xl">
            <h2 className="font-display text-3xl md:text-4xl font-bold text-center mb-10">
              {t("delphiTour.whatIncludedTitle")}
            </h2>
        ...
        <section className="py-12 md:py-16 px-4 bg-card">
          <div className="container mx-auto max-w-4xl">
            <h2 className="font-display text-3xl md:text-4xl font-bold text-center mb-10">
              {t("delphiTour.infoTitle")}
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-5">
              {[
                { icon: Clock, label: t("delphiTour.duration"), val: t("delphiTour.durationVal") },
                { icon: Users, label: t("delphiTour.type"), val: t("delphiTour.typeVal") },
                { icon: MapPin, label: t("delphiTour.departure"), val: t("delphiTour.departureVal") },
                { icon: Wallet, label: t("delphiTour.price"), val: t("delphiTour.priceVal") },
        ...
        <section className="py-16 md:py-20 px-4 bg-card border-t border-primary/20">
          <div className="container mx-auto max-w-2xl text-center">
            ...
            <h2 className="font-display text-2xl md:text-4xl font-bold mb-6">
              {t("delphiTour.title")} · {t("delphiTour.priceVal")}
            </h2>
            <a href={bookUrl} target="_blank" rel="noopener noreferrer">
              <Button
                size="lg"
